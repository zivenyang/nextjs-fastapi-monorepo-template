...
Next.js前端应用新增了用户详情和角色管理功能，用于支持用户信息的查看和编辑，同时提供基于角色的权限控制。

### 功能特性

#### 用户个人资料功能

- 显示当前用户的详细信息，包括基本账户信息和扩展资料
- 支持编辑个人资料，包括用户名、全名、邮箱等基本信息
- 支持更新个人简介、联系方式、头像等扩展信息
- 支持修改密码功能
- 实时表单验证，确保数据符合要求
- 使用Server Actions进行数据更新，保证安全性

#### 用户管理功能 (仅管理员)

- 显示所有用户列表，包括基本信息和状态
- 支持查看任意用户的详细信息
- 角色标识功能，直观显示用户角色(管理员/普通用户/访客)
- 基于角色的权限控制，仅管理员可访问用户管理页面
- 保护非管理员用户无法查看其他用户详情

### 路由结构

- `/profile` - 当前用户的个人资料页面
- `/users` - 用户列表页面 (需要管理员权限)
- `/users/[userId]` - 特定用户的详情页面 (需要管理员权限或查看自己的信息)

### 角色权限系统

系统实现了基于角色的权限控制，主要包括以下角色:

- **管理员 (admin)**: 可以查看和管理所有用户信息
- **普通用户 (user)**: 仅能查看和编辑自己的信息
- **访客 (guest)**: 有限的系统访问权限

权限控制在两个层面实现:

1. **服务器端**: 使用Server Actions进行权限验证，防止未授权访问
2. **客户端**: 根据用户角色动态显示或隐藏UI元素
...
- `GET /api/v1/users/{user_id}` - 获取特定用户信息
...
1. 访问 `/users` 页面查看所有用户
2. 点击特定用户的"查看"按钮，进入用户详情页
3. 在用户详情页可以查看完整的用户信息
4. 管理员可以执行特定管理操作，如修改权限等
